...
class AssetDetailsView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if asset_id_exact != None:
            self.queryset = self.queryset.filter(asset_id=asset_id_exact)
        ...
        if asset_name_exact != None:
            ...
            self.queryset = self.queryset.filter(asset_name__contains=asset_name_contains)
        if asset_type_exact != None:
            self.queryset = self.queryset.filter(asset_id=asset_id_exact)
        elif asset_type_contains != None:
            self.queryset = self.queryset.filter(asset_type__contains=asset_type_contains)
